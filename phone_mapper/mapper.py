def set_none(examples):
    '''Replace the string "None" with Python None in a list.'''
...
# phonemes (CELEX DISC transcription). G ɣ (voiced fricative velar) is
# reported in the README but absent from the matrix, so it is omitted.
...
examples_dutch += ',None'
examples_dutch += ',None,None,None,None,None,None,liep,analyse,None'
...
arpabet_to_ipa = {
    ...
    'AX': 'ə', 'AY': 'ai', 'EH': 'ɛ', 'ER': 'ɝ', 'EY': 'eɪ',
    'IH': 'ɪ', 'IX': 'ɨ', 'IY': 'i', 'OW': 'oʊ', 'OY': 'ɔɪ',
    'UH': 'ʊ', 'UW': 'u', 'UX': 'ʉ',
    'B': 'b', 'CH': 'tʃ', 'D': 'd', 'DH': 'ð', 'DX': 'ɾ',
    'EL': 'l̩', 'EM': 'm̩', 'EN': 'n̩', 'F': 'f', 'G': 'g',
    'HH': 'h', 'H': 'h', 'JH': 'dʒ', 'K': 'k', 'L': 'l',
    'M': 'm', 'N': 'n', 'NX': 'ɾ̃', 'NG': 'ŋ', 'P': 'p',
...
ipa_to_arpabet = {}
for _key, _value in arpabet_to_ipa.items():
    ipa_to_arpabet[_value] = _key
ipa_to_arpabet['n̩'] = 'n'
ipa_to_arpabet['r'] = 'R'
ipa_to_arpabet['x'] = 'X'
...
arpabet_to_examples = {
    'AA': 'b(al)m,b(o)t', 'AE': 'b(a)t', 'AH': 'b(u)tt',
    'AO': 'c(augh)t,st(o)ry', 'AW': 'b(ou)t', 'AX': 'comm(a)',
    'AY': 'b(i)te', 'EH': 'b(e)t', 'ER': 'b(ir)d,forew(or)d',
    'EY': 'b(ai)t', 'IH': 'b(i)t', 'IX': 'ros(e)s,rabb(i)t',
    'IY': 'b(ea)t', 'OW': 'b(oa)t', 'OY': 'b(oy)',
    'UH': 'b(oo)k', 'UW': 'b(oo)t', 'UX': 'd(u)de',
    'B': '(b)uy', 'CH': '(Ch)ina', 'D': '(d)ie', 'DH': '(th)y',
    'DX': 'bu(tt)er', 'EL': 'bott(le)', 'EM': 'ryth(m)',
    'EN': 'butt(on)', 'F': '(f)ight', 'G': '(g)uy', 'HH': '(h)igh',
    'H': '(h)igh', 'JH': '(j)ive', 'K': '(k)ite', 'L': '(l)ie',
    'M': '(m)y', 'N': '(n)igh', 'NX': 'wi(nn)er', 'NG': 'si(ng)',
    'P': '(p)', 'Q': 'uh(-)oh', 'R': '(r)ye', 'S': '(s)igh',
    'SH': '(sh)y', 'T': '(t)ie', 'TH': '(th)igh', 'V': '(v)ie',
    'W': '(w)ise', 'WH': '(wh)y', 'Y': '(y)acht', 'Z': '(z)oo',
    'ZH': 'plea(s)ure',
}
...
coolest_textgrid_phoneme_set = (
    'o:,u,b,g,Au,i,z,f,v,G,x,n,E,Ei,@,y,O,9y,A,e:'
    ',I,t,Y,l,k,a:,w,d,N,r,s,h,p,m'
)
...
ipa_to_dutch_examples = dict(zip(ipa_set, examples_dutch))
ipa_to_english_examples = dict(zip(ipa_set, examples_english))
ipa_to_german_examples = dict(zip(ipa_set, examples_german))
...
class Mapper:
    ...
    def _make_dicts(self):
        '''Create bidirectional dicts for IPA, SAMPA, CELEX, DISC, and CGN.'''
        self.ipa_to_sampa = {}
        self.ipa_to_celex = {}
        self.ipa_to_disc = {}
        self.ipa_to_cgn = {}
        ...
        sets = [self.ipa_set, self.sampa_set, self.celex_set,
            self.disc_set, self.cgn_set]
    ...
    def _add_arpabet(self):
        ...
        for ipa, disc in self.ipa_to_disc.items():
            ...
            if ipa == 'æ̃ː': ipa = 'æ̃'
            if ipa == 'ŋ̩': ipa = 'ŋ'
            if ipa not in self.ipa_to_arpabet:
                continue
...
def validate(mapper=None):
    '''Return a dict of set-name → count for all phoneme sets in mapper.'''
    if not mapper: mapper = Mapper()
    return {name: len(getattr(mapper, name)) for name in mapper.names}


def show(mapper=None):
    '''Print all phoneme sets side by side, one phoneme per row.'''
    if not mapper: mapper = Mapper()
...
# Convenience lookups built from a default Mapper instance
